angular.module('commbox', ['angularFileUpload']).directive('filehandleshadowbox', ['$http', '$q', '$upload', function ($http, $q, $upload) {
    return {
        restrict: 'E',
        ...
        controller: ['$scope', function ($scope) {
            ...
            $scope.FileList = function () {
                ...
                $http.post(gb_allpath + 'aj_FList', { id: $scope.sysid, FileKind: $scope.filekind }).success(function (data, status, headers, config) {
                    $scope.filedata = data.filesObject;
                    console.log('directive get file', $scope.filedata);
                });
            };
            $scope.FileDelete = function (filename) {
                $http.post(gb_allpath + 'aj_FDelete', { id: $scope.sysid, FileKind: $scope.filekind, FileName: filename }).success(function (data, status, headers, config) {
                    if (!data.result) {
                        alert(data.error);
                    }
                    else {
